...
Alternatively, install Qt Creator and open the `InCoin-qt.pro` file.

An executable named `InCoin-qt` will be built.
...
A warning for people using the *static binary* version of InCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of InCoin is linked against libdb5.3.
...
installed causes InCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
