...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #InCoinTEST2\r");
            Send(hSocket, "WHO #InCoinTEST2\r");
        } else {
            // randomly join #InCoin00-#InCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #InCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #InCoin%02d\r", channel_number).c_str());
        }
