...
unsigned int nStakeMinAge = 1 * 24 * nOneHour; //  days as zero time weight
unsigned int nStakeMaxAge = 15 * 24 * nOneHour; //  days as full weight
unsigned int nStakeTargetSpacing = 8 * 60; // 8-minute stakes spacing
int64_t nTargetSpacing = 8 * 60;  // Same as the above
...
const string strMessageMagic = "InCoin Signed Message:\n";
...
{
    int64_t nSubsidy = 1800 * COIN;
	
	if (nHeight == 1)
		return 256000000 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 300000;
    if (halvings >= 64)
        return nFees;
...
static const int64_t nTargetTimespan = 8 * 15 * 60;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in InCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // InCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "InCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "IN GENESIS";
        CTransaction txNew;
        txNew.nTime = 1476105017;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1476105017;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4163009; // !fTestNet ? 1575379 : 46534;
...
unsigned char pchMessageStart[4] = { 0xb4, 0xf4, 0xe4, 0xe2 };
