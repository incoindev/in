...
{
    resize(850, 550);
    setWindowTitle(tr("InCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a InCoin address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About InCoin"), this);
    aboutAction->setStatusTip(tr("Show information about InCoin"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for InCoin"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your InCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified InCoin addresses"));
    secondAuthAction = new QAction(QIcon(":/icons/key"), tr("Second &auth..."), this);
    secondAuthAction->setStatusTip(tr("Second auth with your InCoin addresses"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("InCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("InCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to InCoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid InCoin address or malformed URI parameters."));
    }
